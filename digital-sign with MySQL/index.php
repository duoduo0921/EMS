...
<?php require_once 'addReservation.php'; ?>
...
//create a DB in PHPMyAdmin, and create a digitalSign table and of course insert data
...
if($result->num_rows > 0){
    ...
    while($row = $result->fetch_assoc()){
        $howlong = $row["blength"] / 60 .' Hour';
        if(strtotime($row["starttime"] + $row["blength"] * 60) < strtotime($time)) {
            $sql = "DELETE FROM `Bookings` WHERE bookingid = 1"; 
            $mysqli->query($sql) or die($mysqli->error); 
        }
    }
} 

//if no id entered, show room 109 as default
if($_GET['id']) {
    $id = $_GET['id'];
} else {
    $id = 109;
...
if (sizeof($listallbookings) == 0) {
  ...
  if(!$quickbooked) {
    echo "<div style=\"margin-left: 20%;margin-top: 3%\">
    <form action=\"addReservation.php\" method=\"post\">
    <button type=\"submit\" name=\"addOne\" class=\"btn btn-info\">Book the Room For An Hour</button>
    <button type=\"submit\" name=\"addOneHalf\" class=\"btn btn-info\">Book the Room For One and a Half Hour</button>
    <button type=\"submit\" name=\"addTwo\" class=\"btn btn-info\">Book the Room For Two Hours</button>
    </form>
</div>";
  } else {
    echo "<div class=\"alert alert-info col-xs-12 col-sm-10 col-md-8 offset-md-2 offset-sm-1\" role=\"alert\">
    <h1 class=\"alert-heading text-center\">Just Quick Booked For $howlong </h1>
</div>";
  }
} else {
        ...
        foreach ($uniqueBookings as $key=>$value) {
            ...
            if (check_in_range(date("Y-m-d\TH:i:s"),date("Y-m-d\T23:59:59"),$eventstart)) { // is in today
                if(getDifference(date("Y-m-d\TH:i:s"), $eventstart) < 1) {
                    ...
                    if(!$quickbooked) {
                        echo "<div style=\"margin-left: 20%;margin-top: 3%\">
                    <button type=\"button\" class=\"btn btn-info\">Book the Room For An Hour</button>
                        </div>";
                    }
                } else {
                    ...
                    if(!$quickbooked) {
                        echo "<div style=\"margin-left: 20%;margin-top: 3%\">
                    <button type=\"button\" class=\"btn btn-info\">Book the Room For An Hour</button>
                    <button type=\"button\" class=\"btn btn-info\">Book the Room For One and a Half Hour</button>
                    <button type=\"button\" class=\"btn btn-info\">Book the Room For Two Hours</button>
                        </div>";
                    }
                }
...
<!-- <form action="addReservation.php" method="post">
    <button type="submit" class="btn btn-info">Book Test</button>
</form> -->
